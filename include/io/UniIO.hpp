...
public:
	...
	virtual regsize_t in(addr_t reg_addr_port, iosize_t n) noexcept =0;    //从指定的寄存器/地址/端口中读入n个字节，不可超过regsize_t的大小(允许各个实现固定输入字节大小)
    virtual void        out(addr_t reg_addr_port, regsize_t data, iosize_t n) noexcept =0; //向指定的寄存器/地址/端口中写入n个字节，不可超过regsize_t的大小(允许各个实现固定输出字节大小)
};
