...
inline int localpos(unsigned int pos) noexcept{
    ...
    assert(pos < sizeof(regsize_t));

    /* 物理机是小端机器时，可直接返回小端位置 */
#if __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN_
    return pos;
#else
    return (sizeof(regsize_t) - 1) - pos;
#endif
}

/* 以大小端无关方式获取src指定位置的一个字节 */
inline charunit_t getbyte(regsize_t src, unsigned int pos) noexcept{
    charunit_t *p = reinterpret_cast<charunit_t*>(&src) + localpos(pos);
    return *p; 
}

/* 以大小端无关方式将字节b设置到src指定位置，返回设置后的src */
inline regsize_t setbyte(regsize_t src, charunit_t b, unsigned int pos) noexcept{
    charunit_t *p = reinterpret_cast<charunit_t*>(&src) + localpos(pos);
    *p = b;
    return src;
}
...
inline regsize_t clrbyte(regsize_t src, unsigned int pos) noexcept{
    charunit_t *p = reinterpret_cast<charunit_t*>(&src) + localpos(pos);
    *p = 0;
    return src;
}
